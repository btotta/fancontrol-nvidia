...
#   enable nvidia coolsbits to allowed fan control and overclock - needed for this script to work
...
def get_fan_speed():
    return int(subprocess.getoutput(
        'nvidia-settings -q [fan:0]/GPUTargetFanSpeed -t'))


def set_fan(speed):
    if get_fan_speed() != speed:
        subprocess.getoutput(
            'nvidia-settings -a GPUFanControlState=1 -a GPUTargetFanSpeed={}'
            .format(speed))
...
while True:
    ...
    if current_temp >= temp_curve[-1]:
        ...
        for index, item in enumerate(temp_curve):
            ...
            if temp_curve[index] <= current_temp < temp_curve[index + 1]:
